...
model = "TChiSlepSnu_tauenriched_x0p05"
...
diag_low, diag_high = 150, 25
xmin, xmax, xstep = 100, 1200, 50
ymin, ymax, ystep_low, ystep_high = 0, 500, 50, 25 
...
for mx in range(xmin, xmax+1, xstep):
  ...
  if mx > (ymax + (diag_low - diag_high)): 
    ...
    ylist.extend(range(ymin, mx - diag_low, ystep_low))
    if mx - diag_low > 0: ylist.extend(range(mx - diag_low, mx-diag_high+1, ystep_high))
    else:  ylist.extend(range(0, mx-diag_high+1, ystep_high))
  for my in ylist:
